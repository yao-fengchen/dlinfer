...
class AtbOverrides:
    ...
    def LinearParallel(name, x, weight, bias, dp_gather, all_reduce, rank, tp_size, group):
        op = Operation(name, "LinearParallelOperation")
        param = infer_param.LinearParallelParam()
        ...
        print(f"param.rankTableFile: {param.rankTableFile}, param.commDomain: {param.commDomain}", flush=True)
        if bias:
            op.set_input([x, weight, bias])
        else:
            op.set_input([x, weight])
