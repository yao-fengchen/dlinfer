...
AclNnBincountOperation::AclNnBincountOperation(const std::string& name, bool hasWights, int64_t minlength) : AclNnOperation(name), minlength_(minlength) {}
...
uint32_t AclNnBincountOperation::GetInputNum() const { return hasWights ? NUM2 : NUM1; }
...
int AclNnBincountOperation::SetAclNnWorkspaceExecutor(uint64_t& workspaceSize) {
    ...
    int ret = aclnnBincountGetWorkspaceSize(
        aclInTensors_.at(0).tensor, hasWights ? aclInTensors_.at(1).tensor : nullptr, minlength_, aclOutTensors_.at(0).tensor, &workspaceSize, &aclExecutor_);
...
atb::Operation* AclNnBincountOperationCreate(const nlohmann::json& paramJson) {
    ...
    if (paramJson.contains("hasWights")) {
        hasWights = paramJson["hasWights"].get<bool>();
    }
    ...
    atb::Operation* op = new AclNnBincountOperation(opName, hasWights, minlength);
    return op;
}
