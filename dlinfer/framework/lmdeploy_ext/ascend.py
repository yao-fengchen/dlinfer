import lmdeploy.lite.quantization.modules.linear as lmdeploy_lite_linear
...
def AscendMergedAwqLinear__init__(self,
                in_features: int,
    ...
    super(MergedAwqLinear, self).__init__(in_features,
                        out_features,
                        w_bit,
                        group_size,
                        bias,
...
def weight_loader(self, param: torch.nn.Parameter,
    ...
    try:
        ...
        print(self.all_out_features, loaded_weight.shape, param.name, flush=True)
...
def create_weights(self, in_features: int, out_features: int, w_bit: int,
    ...
    scales = torch.empty((grouped_in_feats, out_features),
                            dtype=dtype,
                            device=device)
    qzeros = torch.empty((grouped_in_feats, out_features),
                            dtype=dtype,
                            device=device)
    if bias:
...
WeightOnlyQLinear.from_linear = classmethod(from_linear)
MergedAwqLinear.__init__ = AscendMergedAwqLinear__init__
MergedAwqLinear.weight_loader = weight_loader
WeightOnlyQLinear.__init__ = AscendWeightOnlyQLinear__init__
AwqLinear.create_weights = create_weights
