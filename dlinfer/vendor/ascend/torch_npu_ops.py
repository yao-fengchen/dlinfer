...
def prefill_attention(
    ...
    if SocVersion.is_Ascend910B():
        ...
        attn_output_shape = query.shape[:-1] + (value.shape[-1], )
        attn_output.view(attn_output_shape)[:] = torch.ops.npu.npu_fusion_attention(
            query,
            key,
            value,
            num_q_heads,
...
def paged_decode_attention(
    ...
    attn_output_shape = query.shape[:-1] + (value_cache.shape[-1], )
    if is_mla:
        import torch_npu
        ...
        torch.ops.npu_ext.npu_incre_flash_attention_v4_out(
            ...
            key_cache,
            value_cache,
            attn_output.view_as(attn_output_shape),
            padding_mask=None,
            atten_mask=None,
